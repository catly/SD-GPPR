...
def run():
    ...
    label_feature_word_sorted = defaultdict(list)
    select_num = 0
    ...
    for l, word_freq in label_corpus_word_freq.items():
        if l!="NONE":
            ...
            # k = 1259
            # k = 900
            tmp = sorted(word_ctfidf.items(), key=lambda x: x[1], reverse=True)[:k]
            print('*****')
            print(tmp)
            # k = 0.50
            # select_num = select_num + int(len(word_ctfidf) * k)
            # tmp = sorted(word_ctfidf.items(), key=lambda x: x[1], reverse=True)[:int(len(word_ctfidf) * k)]
            # select_num += int(len(word_ctfidf) * k)
            # print(int(len(word_ctfidf) * k))

            # 前1259*3=3777个词 没有交集

            # print(len(word_ctfidf.items()))
            '''
            NONE
            2889
            FAVOR
            ...
            2889+2660+4009
            '''
            label_feature_word_sorted[l] = [w for (w, s) in tmp]
            # print(label_feature_word_sorted[l])
            label_feature_word[l] = set(w for (w, w_c_tfidf) in tmp)
    # print(label_feature_word)
    w_set = []
    for l, l_w_list in label_feature_word_sorted.items():
        w_set = w_set + l_w_list
    print('selected words list num:' + str(select_num))
    print('selected words set num:' + str(len(set(w_set))))  # ==3*k  ??
    with open('feature_words.txt', 'w') as f_w:
        for w in w_set:
            f_w.write(str(w) + ' ')

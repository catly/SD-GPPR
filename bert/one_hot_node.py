...
def train():
    ...
    # word-idx
    words_dict = load_json("./dataset/word2idx.json")
    word_len = len(words_dict)
    # for i in range(word_len):
    #     label_list.append('WORD')

    texts_e = []
    for i in range(node_len):
        ebeddings = torch.tensor([0]*(word_len))
        node_words = node_list[i] + ' ' + target_list[i]
        for w in node_words.split(' '):
            if w in words_dict.keys():
                ebeddings[words_dict[w]] = 1
            else:
                ebeddings[1] = 1
        texts_e.append(ebeddings)

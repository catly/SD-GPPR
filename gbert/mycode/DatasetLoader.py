...
from mycode.base_class.dataset import dataset
import torch
import numpy as np
...
class DatasetLoader(dataset):
    ...
    def encode_onehot(self, labels):
        if self.dataset_name == 'sem16':
            classes = ['AGAINST','FAVOR','NONE','WORD']
        else:
            classes = set(labels)
    ...
    def load(self):
        ...
        if self.dataset_name == 'cora':
            ...
            idx_val = range(5, 10)
            idx_test = range(5, 10)
        elif self.dataset_name == 'sem16':
            idx_train = range(1249, 4063)
            idx_val = range(4063, 4163)
            idx_test = range(0, 1249)
